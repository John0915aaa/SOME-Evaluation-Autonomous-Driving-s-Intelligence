"""Generate AV-focused prompts for crossing scenarios."""
...
def calculate_indicator(index_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for rank, (idx, row) in enumerate(index_df.iterrows(), start=1):
        ...
        AV_end_x, AV_end_y = ego_end_state[column_dict["x"]], ego_end_state[column_dict["y"]]
        ...
        HV_end_x, HV_end_y = agent_end_state[column_dict["x"]], agent_end_state[column_dict["y"]]
        HV_end_dis = collision_point.distance(Point(HV_end_x, HV_end_y))

        if AV_end_dis >= HV_end_dis:
            nearest_agent = f"HV距离冲突点最近, 为{HV_end_dis:.2f}米, ego距离冲突点{AV_end_dis:.2f}米"
        else:
            nearest_agent = f"ego距离冲突点最近, 为{AV_end_dis:.2f}米, HV距离冲突点{HV_end_dis:.2f}米"
        sentence = f"""\n第{interaction_end - interaction_start - 1}秒时, 该交互片段结束, {nearest_agent}."""
        end_time_index = interaction_end - interaction_start - 1
        ...
        for time_index, timestamp in enumerate(all_timesteps):
            ...
            if time_index > 0:
                ...
                if np.dot(AV_v1, AV_v2) < 0:
                    sentence = f"""\n第{time_index}秒后, ego率先通过了冲突点, 交互结束, 此时HV距离冲突点{HV_dis:.2f}米, 预计将在{(HV_dis/HV_v):.2f}秒后通过冲突点."""
                ...
                if np.dot(HV_v1, HV_v2) < 0:
                    sentence = f"""\n第{time_index}秒后, HV率先通过了冲突点, 交互结束, 此时ego距离冲突点{AV_dis:.2f}米, 预计将在{(AV_dis/AV_v):.2f}秒后通过冲突点."""
                    end_time_index = time_index
                    break
        ...
        for time_index, timestamp in enumerate(all_timesteps):
            ...
            row = 0.6
            t_AV_pass = (AV_dis + length / 2 + row) / AV_v
            t_AV_arrive = (AV_dis - length / 2 - row) / AV_v
            t_HV_pass = (HV_dis + length / 2 + row) / HV_v
            t_HV_arrive = (HV_dis - length / 2 - row) / HV_v
        ...
        data = {"indexs": index, "prompt": prompt, "PET_min": min_PET}
