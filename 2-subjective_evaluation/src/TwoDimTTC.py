...
def dist_p2l(point, line_start, line_end):
    ...
    ) / np.sqrt((line_end[0] - line_start[0]) ** 2 + (line_end[1] - line_start[1]) ** 2)
...
def CurrentD(samples, toreturn="dataframe"):
    if toreturn != "dataframe" and toreturn != "values":
        ...
        dist_mat = []
        count_i = 0
        for point_i_start, point_i_end in zip(
            [point_i1, point_i4, point_i3, point_i2], [point_i2, point_i3, point_i1, point_i4]
        ):
            count_j = 0
            for point_j_start, point_j_end in zip(
                [point_j1, point_j4, point_j3, point_j2], [point_j2, point_j3, point_j1, point_j4]
            ):
                if count_i < 2 and count_j < 2:
                    # Distance from point to point
                    dist_mat.append(
                        np.sqrt((point_i_start[0] - point_j_start[0]) ** 2 + (point_i_start[1] - point_j_start[1]) ** 2)
                    )
                    dist_mat.append(
                ...
                ist = intersect(
                    line(
                        point_i_start,
                        point_i_start + np.array([-(point_j_start - point_j_end)[1], (point_j_start - point_j_end)[0]]),
                    ),
                ...
                dist[
                    np.isnan(ist[0])
                    & (ison(point_i_start, point_i_end, point_j_start) | ison(point_i_start, point_i_end, point_j_end))
                ] = 0
...
def TTC_ij(samples):
    ...
    for point_line_start in [point_i1, point_i2, point_i3, point_i4]:
        for edge_start, edge_end in zip(
            [point_j1, point_j3, point_j1, point_j2], [point_j2, point_j4, point_j3, point_j4]
        ):
    ...
    vxi = samples["vx_i"].values
    vyi = samples["vy_i"].values
    vxj = samples["vx_j"].values
    vyj = samples["vy_j"].values
    ...
    ttc = dist2overlap / np.sqrt((vxi - vxj) ** 2 + (vyi - vyj) ** 2)
    # print(f"ttc1 = {type(ttc)}")
...
def TTC(samples, toreturn="dataframe"):
    if toreturn != "dataframe" and toreturn != "values":
        ...
        if toreturn == "dataframe":
            ...
            # print(f"ttc_ij = {ttc_ij}, ttc_ji = {ttc_ji}")
