    ...
    def extract_interactions(self, time_dic):
        ...
        for time_step, dic in time_dic.items():
            ...
            for pair in unique_tuples:
                ...
                merged_tuple = tuple(
                    sorted(set(element for sub_pair in interaction_info[pair].keys() for element in sub_pair))
                )

                # Update the result dictionary with the merged tuple and corresponding data
                result_dict[time_step][merged_tuple] = {
                    sub_pair: sub_distances for sub_pair, sub_distances in interaction_info[pair].items()
                }
...
class AccelerationCalculate(MultiProcess):
    ...
    def extract_motion(self, agent_states, agent_id, timestamp, index_x, index_y, index_vx, index_vy):
        ...
        # motion包含：x y 坐标以及在x y处的沿x y坐标轴方向的速度
        motion = (current_motion[index_x], current_motion[index_y], current_motion[index_vx], current_motion[index_vy])

        # Calculate the speed as the magnitude of the velocity vector
        speed = math.sqrt(motion[2] ** 2 + motion[3] ** 2)  # 计算速度标量
    ...
    def get_acceleration(self, time_dict_scenario, agent_states, all_agents, column_dict, all_timesteps):
        ...
        for timestamp_now in self.all_timesteps:
            ...
            for (
                pair_multi,
                pair_dis,
            ) in current_time_data.items():  # pair_multi represents the group of interacting agents
                pair_info = {}
                ...
                for (id_1, id_2), (dis1, dis2, _, t_diff) in pair_dis.items():
                    ...
                    motion1 = self.extract_motion(
                        self.agent_states, id_1, timestamp_now, index_x, index_y, index_vx, index_vy
                    )
                    motion2 = self.extract_motion(
                        self.agent_states, id_2, timestamp_now, index_x, index_y, index_vx, index_vy
                    )
                ...
                a_min, success, solution, min_pet, mean_pet = self.MultiCalculateAcceleration(
                    pair_info, pair_multi, timestamp_now
                )
