...
try:
    ...
    from .AccelerationCalculator import MultiProcess, AccelerationCalculate
...
class SceneProcessor:
    ...
    def __init__(
        ...
        self.all_agents = {agent.name: agent for agent in desired_scene.agents}
    ...
    def process_scene(
        self, time_dic_output: Dict, a_min_dict: Dict, multi_a_min_dict: Dict, delta_TTCP_dict: Dict, results: Dict
    ) -> List[List]:
        ...
        intersectiondetector = IntersectionDetector(
            self.agent_states, self.all_timesteps, self.column_dict, self.all_agents, move_agent, map_processed_data
        )
    ...
    def get_fut_line(self, move_agent: Dict[int, List[str]]) -> Dict[int, Dict[str, Dict[str, Any]]]:
        ...
        for timeindex, timestamp in enumerate(self.all_timesteps):
            ...
            for agent_id in current_ids:
                ...
                trajectory, future_time, x, y, complete_lane, speed, distance = self.process_vehicle_tracks(
                    track_idx, timeindex, lanes
                )
                ...
                map_processed_data[timestamp][agent_id] = self.process_tracks(
                    ...
                    [self.column_dict["vx"], self.column_dict["vy"]],
                    self.dt,
    ...
    def get_results(self, timestamp_msaa: Dict[int, Dict[Tuple[int, int], List[float]]]) -> List[List]:
        ...
        for pair, value in result_segments.items():
            ...
            if end_time - start_time > 3:
                ...
                msaa_list = [
                    timestamp_msaa[ts].get(pair, [0, None, None, None])[0] for ts in range(start_time, end_time + 1)
                ]
                intensity = max(msaa_list)
                mean_pet_list = [
                    timestamp_msaa[ts].get(pair, [None, None, None, 0])[3] for ts in range(start_time, end_time + 1)
                ]
    ...
    def get_segments(
        self, row_labels: List, col_labels: List, a_min_matrix: np.ndarray, acceleration_thresh: float
    ) -> Dict:
    ...
    def find_last_nonzero_index(
        self, agent_states: np.ndarray, track_id_index: int, position_index: List[int]
    ) -> Optional[int]:
    ...
    def process_tracks(
        ...
        v_index: List[int],
        dt: float,
        additional_trajectory: Optional[np.ndarray] = None,
    ) -> Dict[str, Any]:
    ...
    def get_complete_lane1(
        self,
        x: float,
        y: float,
        h: float,
        center_lane_id: int,
        downstream_lane_ids: List[int],
        vec_map: VectorMap,
        lane_ids: List[int],
    ) -> np.ndarray:
    ...
    def get_complete_lane(
        ...
        # 找到中心车道中离 (x, y) 最近的点，并截取其后的部分
        center_points = center_lane.points
        distances = np.linalg.norm(center_points[:, :2] - np.array([x, y]), axis=1)
        closest_index = np.argmin(distances)
        truncated_center_lane = center_points[closest_index:]

        # 添加起始路段
        lane_points.append(truncated_center_lane)

        # 如果有下游车道，则补充到 lane_points 中
        if downstream_lane_ids:
            for lane_id in downstream_lane_ids:
                if lane_id in lane_ids:
                    downstream_lane = vec_map.get_road_lane(lane_id).center
                    if downstream_lane:
                        lane_points.append(downstream_lane.points)
        # print(f"complete lane: {lane_points[0][:5]}")

        return np.vstack(lane_points)

    def extract_path_from_current_position(
        self, x: float, y: float, lane_points: np.ndarray, distance: float
    ) -> np.ndarray:
    ...
    def process_vehicle_tracks(self, track_id_index: int, timestamp_index: int, lanes: List) -> Tuple:
        ...
        """
        # print(f"track_id_index:{track_id_index}")
        # Use self to access all_tracks, column_dict, and other attributes
        all_tracks = self.agent_states
        # print(f"all_tracks: {all_tracks}")
        column_dict = self.column_dict
        time_stamp_list = self.all_timesteps
        """ print(f"time_stamp_list: {time_stamp_list}")
        print(f"time_stamp_list.index(max(time_stamp_list)): {time_stamp_list.index(max(time_stamp_list))}") """
        ...
        if len(lanes[1]):
            ...
            # print(f"complete_lane:{complete_lane}")
        ...
        # print(f"x, y: {x, y}")
        # print(f"complete lane: {complete_lane}")
