...
try:
    from .trajdata_utils import DataFrameCache, get_agent_states
except ImportError:
...
def process_tracks_single(
    considertime, all_tracks, track_id_index, timestamp_index, timerange, position_index, v_index
):
    """
    ...
    # print(f"time = {timestamp_index}, state = {v_x, v_y}")
...
def polygon_xy_from_motionstate(x, y, psi_rad, width=width, length=length):
    """
    ...
    rotated_corners = rotate_around_center(corners, np.array([x, y]), yaw=psi_rad)
...
def plot_lanes(plt, vec_map, view_rect):
    ...
    for laneid in vec_map.lanes:
        ...
        if right_lane is not None and is_line_in_view(right_lane.points, view_rect):
            plt.plot(
                right_lane.points[:, 0], right_lane.points[:, 1], "-", markersize=1, color="#969696", linewidth=0.4
            )
...
def plot_agent_trajectory(
    ...
    x, y = (
        agent_states[id_index, timestamp_index, column_dict["x"]],
        agent_states[id_index, timestamp_index, column_dict["y"]],
    )
    ...
    if agent_id in interact_ids:
        ...
        plot_interacting_agent(line_x, line_y, x, y, psi_rad, ego_index, id_index, key_agent_index)
    else:
        plot_non_interacting_agent(line_x, line_y, x, y, psi_rad)


def plot_interacting_agent(line_x, line_y, x, y, angle, ego_index, id_index, key_agent_index):
    """
    ...
    if id_index == ego_index:
        ...
        plt.plot(xnew, ynew, "-", color="red", zorder=10, linewidth=1)

        # Draw the agent's polygon based on its current position and orientation
        rect = patches.Polygon(polygon_xy_from_motionstate(x, y, angle), closed=True, zorder=20, color="red")
        plt.gca().add_patch(rect)  # Add the polygon patch to the plot
    elif id_index == key_agent_index:
        # Interpolate the trajectory line using a linear interpolation function
        f = interp1d(line_x, line_y, kind="linear")
        # Generate new x-values for a smooth line, with 1000 points between min and max of line_x
        xnew = np.linspace(min(line_x), max(line_x), num=1000, endpoint=True)
        ...
        # Plot the interpolated trajectory line
        plt.plot(xnew, ynew, "-", color="green", zorder=10, linewidth=1)

        # Draw the agent's polygon based on its current position and orientation
        rect = patches.Polygon(polygon_xy_from_motionstate(x, y, angle), closed=True, zorder=20, color="green")
        plt.gca().add_patch(rect)  # Add the polygon patch to the plot
    else:
        # Interpolate the trajectory line using a linear interpolation function
        f = interp1d(line_x, line_y, kind="linear")
...
def draw_pic(
    ...
    timestamp_index = all_timesteps.index(timestamp)  # timestamp_index = 0, 1, 2, ...
    ...
    x_min, x_max, y_min, y_max = setup_plot_bounds(
        interact_ids, mean_time, scene_cache, column_dict, vec_map, all_agents, all_timesteps, dt, considertime
    )
    ...
    current_ids = [
        all_agents[index] for index, flag in enumerate(agent_states[:, timestamp_index, column_dict["vx"]]) if flag != 0
    ]
