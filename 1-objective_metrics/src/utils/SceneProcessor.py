...
from src.utils.trajdata_utils import DataFrameCache
from src.utils.AccelerationCalculator import MultiProcess, AccelerationCalculate
from src.utils.IntersectionDetector import IntersectionDetector
...
class SceneProcessor:
    ...
    def get_agent_states(self):
        ...
        x_index, y_index, z_index = self.column_dict['x'], self.column_dict['y'], self.column_dict['z']
    ...
    def get_results(self, timestamp_msaa: Dict[int, Dict[Tuple[int, int], List[float]]]) -> List[List]:
        ...
        for pair, value in result_segments.items():
            ...
            if end_time - start_time > 3:
                ...
                ego_type = 'AV_1' if 'ego' in pair else 'AV_2'

                # Append the extracted data to the results list
                results.append([
                    col_dataset, col_scenario_idx, track_idx, start_time,
                    end_time, intensity, PET, two_multi, vehicle_type, ego_type
                ])
    ...
    def get_complete_lane1(self, x: float, y: float, h: float, center_lane_id: int,
