...
def select_ego_and_key_agent(row: pd.Series, interact_ids: list[str]) -> tuple[str, str]:
    ...
    elif row["ego_type"] == "AV_2":
        # In AV_2 scenes the first track_id is the evaluated AV, regardless of
        # the ordering (or membership) of key_agents.
        ego_id = interact_ids[0]
        key_agent = next(agent for agent in key_agents if agent != ego_id)
    else:
        raise ValueError(f"Unknown ego_type: {row['ego_type']!r}")
